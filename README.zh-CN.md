...
在消息区按 **`L`** 切换发出消息布局（`transcript` = 左对齐并带 `>` 前缀，`im` = 收到左 / 发出右）。在选中消息上按 **`R`** 查看反应并快速发送 emoji（键 `1`–`8`）。广播频道帖子显示浏览次数（👁），而非私聊已读回执。
...
撰写建议限制：面板最多显示 5 条可见条目。需要位置信息的内联 Bot 在本 MVP 中标记为不支持。
