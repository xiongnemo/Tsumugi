...
- 本地留多少历史由两个设置项决定，都在 设置 → 存储 里，旁边直接显示当前数据库大小：
  - 「保留历史（天数，0 = 永久）」（`TSUMUGI_RETENTION_DAYS`，默认 60 天）。清理在后台进行，永远不会清空一个会话（每个会话至少保留最新 200 条），也绝不删除未发送成功的消息。
  - 「预取历史（天数，0 = 关闭）」（`TSUMUGI_BACKFILL_DAYS`，默认 7 天）。后台回填只把最近活跃的会话补到这个深度然后停下，并且始终比保留窗口更浅，两者不会互相打架。设为 0（或保留时长本身足够短）时预取完全关闭，往前翻改为按需加载——官方客户端就是这么做的：它们的本地数据库是「你打开过什么」的缓存，而不是历史的副本。之前没有界限，它永远不会结束——在真实账号上一天就存了六百万条消息、3.7 GB 数据库。
  - 「立即清理」把这两个设置应用到已经存下来的数据：先删掉超出保留窗口的消息，再重写数据库，把空间真正还给文件系统。只删不重写是缩不小文件的——SQLite 会把腾出来的页留着自己复用——所以这是让已经涨大的数据库瘦回去的唯一办法。重写期间其他操作需要等待，数据库较大时可能需要数分钟。
- 单行状态栏：连接信息（左）、前台网络活动（中）、后台任务（右，仅在有任务时显示）。会话摘要与历史缺口提示显示在消息区标题中。
- 向当前会话发送文本消息，支持从消息操作菜单选择回复目标。
- 发送媒体。**`a`** 打开纯键盘的文件浏览器，右侧实时预览光标所在的图片；**`A`** 以「按文件发送」模式打开，打开后可用 `Ctrl+F` 切换。列表第一行是**剪贴板图片**，截图不用先存成文件就能发。从别处复制来的路径可以直接在输入框里写 `:file <路径>` —— 用冒号而不是斜杠，因为斜杠开头是 Telegram 的 bot 命令，拦截它会静默吞掉一条真消息。待发送的文件显示在输入框边框上（和回复目标并列），`Ctrl+J` 把文件和文字作为同一条消息发出。图片按图片发，GIF 保持动图，其余按文件发并带上原文件名。视频在有 `ffprobe` 时带上宽高与时长，没有时退化成普通文件发送 —— ffmpeg 始终是可选的。上传进度显示在状态栏；上传失败会留下一条可重试的记录，并且记得本地文件在哪。
- 编辑已发送的消息：在自己的消息上按 **`e`**（或从操作菜单选「编辑」）把内容载回输入框，`Ctrl+J` 保存，`Esc` 取消。不做客户端时限判断 —— Telegram 的可编辑窗口按会话类型和是否管理员而异，直接以服务器的答复为准。
- 投票完整渲染：问题、每个选项一行（占比条 + 百分比 + 票数）、底部总票数。投票走操作菜单的「投票」；多选投票用 `Enter` 标记、`Ctrl+J` 提交。答题（quiz）的正确答案在你作答前不会显示。
- 置顶消息：操作菜单里的「置顶」/「取消置顶」，按当前状态显示对应那一项。置顶是静默的 —— 服务消息照常出现，但不会从终端里去打扰上千个成员。
- 新消息提醒，在 设置 → 通用：关闭 / 终端响铃 / 响铃 + 桌面通知（需终端支持）。响铃走终端本身，裸控制台也有效；自己发的消息、当前正在看的会话、已静音的会话都不会响，且一波消息只响一次。桌面通知需要手动开启，并且在 `TERM=linux` 下绝不发送。
- 会话操作按 **`m`**：静音、置顶会话、归档、退出，每一项都显示当前状态的反面。已静音的会话在列表里有标记。退出会先确认，且只对群组和频道提供。
- 语音和音频用 **`O`** 播放（菜单里显示为「播放」）。有桌面环境时交给系统播放器；裸控制台下退回 `mpv`、`ffplay`、`paplay` 或 `aplay` —— 因为 `xdg-open` 在那里会返回成功但什么都不放。
...
- **没有鼠标**。所有浮层和内联结果网格都完全可用键盘操作。设置与代理面板里，`Tab` 或 `→` 进入右侧分区，`↑↓` 在字段和按钮之间移动，`←` 退回分类列表，`Esc` 关闭面板。tview 自带的表单只认 `Tab`，所以在此之前右半边进得去但走不动、也退不出来。
